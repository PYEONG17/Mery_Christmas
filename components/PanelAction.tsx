  ...
  return (
    ...
    >
      <div className="relative w-full max-w-4xl aspect-[4/3] md:aspect-video border-8 border-black bg-sky-900 shadow-[10px_10px_0px_0px_#222] overflow-hidden">
        ...
        <div
          className={`absolute bottom-0 left-[-2rem] md:left-10 transition-transform duration-300 ${
            !thrown ? "cursor-pointer hover:scale-105" : ""
          } z-10`}
          onClick={handleThrow}
          style={{
            // Combine the parallax translate with the hover scale/thrown transform logic
            transform: `${thrown ? "scaleX(-1)" : ""} translate(${
              mousePos.x * -30
            }px, ${mousePos.y * -20}px)`,
          }}
        >
          ...
          <div className="relative w-48 h-60 md:w-80 md:h-96">
            ...
            {/* The Gift */}
            <div
              className={`absolute top-1/2 right-0 w-12 h-12 md:w-24 md:h-24 z-20 ${
                thrown ? "comic-throw" : "breathe"
              }`}
            >
        ...
        {!thrown && (
          <div
            className="absolute top-16 left-4 md:top-20 md:left-40 max-w-[200px] md:max-w-xs z-30 transition-transform duration-75"
            style={{
